    ...
    try:
        with conn.cursor() as cur:
            ...
            if (row := cur.fetchone()) is None:
                raise ValueError("No statistics found in the database.")
    finally:
        conn.close()
...
def get_feedback_stats():
    ...
    try:
        with conn.cursor() as cur:
            ...
            """)
            if (row := cur.fetchone()) is None:
                raise ValueError("No feedback statistics found in the database.")
    finally:
        conn.close()

    return FeedbackStats(thumbs_up=row[0], thumbs_down=row[1])
